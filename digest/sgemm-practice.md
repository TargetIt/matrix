...
- **在第 1 轮（k=0）时：**
  ...
  👉 **左手 4 个数，右手 4 个数，互相配对相乘（外积 Outer Product）**。$4 \times 4 = 16$！他一口气往桌上的 16 个空盒子里各扔进去 1 个计算结果！
...
- 所以，他拿 $A$ 的总次数是：**4 轮 $\times$ 每轮 4 个 = 16 次！**
...
- **组装（外积计算）**：
  ...
  - 左手第1个数(1) $\times$ 右手第1个数(5) = 5 $\rightarrow$ 扔进 `acc[0][0]`
  - 左手第1个数(1) $\times$ 右手第2个数(6) = 6 $\rightarrow$ 扔进 `acc[0][1]`
  - 左手第2个数(3) $\times$ 右手第1个数(5) = 15 $\rightarrow$ 扔进 `acc[1][0]`
  - 左手第2个数(3) $\times$ 右手第2个数(6) = 18 $\rightarrow$ 扔进 `acc[1][1]`
- **第 1 轮结束，盒子的状态变成了**：
  $
...
- **组装（外积计算）并累加到之前的盒子里**：
  - 左手第1个数(2) $\times$ 右手第1个数(7) = 14 $\rightarrow$ 加上原来的 5，`acc[0][0]` 变成 **19**
  - 左手第1个数(2) $\times$ 右手第2个数(8) = 16 $\rightarrow$ 加上原来的 6，`acc[0][1]` 变成 **22**
  - 左手第2个数(4) $\times$ 右手第1个数(7) = 28 $\rightarrow$ 加上原来的 15，`acc[1][0]` 变成 **43**
  - 左手第2个数(4) $\times$ 右手第2个数(8) = 32 $\rightarrow$ 加上原来的 18，`acc[1][1]` 变成 **50**
- **第 2 轮结束，盒子的状态变成了**：
  $
...
- $C_{0,0} = (A 的第 0 行) \times (B 的第 0 列) = 1 \times 5 + 2 \times 7 = 5 + 14 = 19$ ✅
- $C_{0,1} = (A 的第 0 行) \times (B 的第 1 列) = 1 \times 6 + 2 \times 8 = 6 + 16 = 22$ ✅
- $C_{1,0} = (A 的第 1 行) \times (B 的第 0 列) = 3 \times 5 + 4 \times 7 = 15 + 28 = 43$ ✅
- $C_{1,1} = (A 的第 1 行) \times (B 的第 1 列) = 3 \times 6 + 4 \times 8 = 18 + 32 = 50$ ✅
