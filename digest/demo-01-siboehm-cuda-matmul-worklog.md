...
```
矩阵 C (16×16, GMEM)
└── Grid: 4×4 = 16 blocks (gridDim=(4,4))
...
```
矩阵 C (16×16, GMEM)
└── Grid: 16×1 = 16 blocks (gridDim=(16,16), 1D layout)
...
矩阵 C (16×16, GMEM)
└── Grid: 2×2 = 4 blocks
    └── Block: BM=8, BN=8, BK=4 (blockDim=(4,4,2)? adjust for warp size)
        ├── [GMEM→SMEM] As[8×4] + Bs[4×8], double buffered
        ├── Warp Tile: WM=4, WN=4 (warp 计算 4×4 区域)
        │   └── Thread Tile: TM=4, TN=4 (每个线程 4×4=16 元素)
        │       └── 循环 WM_ITER=2 × WN_ITER=2 → 4 warps/block
        └── Warp 内 32 线程协作加载 SMEM
```
