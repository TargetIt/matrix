...
原始网页：[CUTLASS: Fast Linear Algebra in CUDA C++](https://developer.nvidia.com/blog/cutlass-linear-algebra-cuda/)
...
> **给非专业读者**
>
> CUTLASS 不像前两篇那样从一个简单 kernel 写起。它像一个高级工具箱：你已经知道锤子、螺丝刀、扳手分别干什么，它现在要把这些工具做成一套标准化施工方案。

## 第一性原理：为什么要有“层级”

矩阵乘法有三个维度：M、N、K。C 的每个元素都要沿 K 方向累加。问题是：真实矩阵很大，GPU 的快速存储很小。于是必须分层。

> **公式**：大问题不能一次塞进最快的地方，所以要切成小问题；小问题还要继续切，直到适合 block、warp、thread 和指令。

> **导读补充**
>
> 这就是 CUTLASS 的核心。它不是发明了新公式，而是把“把大矩阵切小、把数据搬近、让线程分工、最后写回”的过程标准化。

> **给非专业读者**
>
> 生活类比：一家全国连锁餐厅不可能让总部厨房给每桌做饭。它会分层：总部定菜谱，城市仓库供货，门店厨房备菜，厨师完成一道菜。每层负责不同尺度的问题。CUTLASS 的 GEMM 层级也是这样。

## 四层 tile：从大矩阵到一条指令

- **Thread Block Tile**：一个 CUDA block 负责 C 的一块大矩形。它从 GMEM 取 A/B tile，并放到 SMEM。
- **Warp Tile**：block 内的多个 warp 再分 C 的子区域。warp 是 GPU 执行调度的重要单位。
- **Thread Tile**：一个线程负责更小的 C 子块，accumulator 放在寄存器。
- **Instruction Tile**：最终映射成具体硬件指令，例如 scalar FMA、WMMA fragment 或其它 dot-product 指令。

> **导读补充**
>
> 你可以把这四层理解成“谁负责哪块 C”。越外层，负责区域越大，主要考虑数据从 GMEM 到 SMEM；越内层，区域越小，主要考虑寄存器和指令效率。

> **深入一点**
>
> 如果写成伪代码，CUTLASS 的层级大致像这样：外层 block 决定 C 的大 tile，中层 warp 决定 C 的子 tile，内层 thread 维护自己的 accumulator，最里面的指令做 FMA 或 MMA。
...
> **导读补充**
>
> 真实 CUTLASS 的 tile 尺寸会大得多，也会根据数据类型和 GPU 架构调整。但第一性原理不变：把 C 切成层层小块，每层刚好匹配一个硬件组织单位。
...
> **导读补充**
>
> 手写 kernel 里你调 BM、BN、BK、TM、TN；CUTLASS 里你配置 policy。两者本质相同：用参数描述分工和数据搬运。区别是 CUTLASS 把这些参数放进模板系统，方便生成和复用很多 kernel 变体。

> **深入一点**
>
> 模板化还有一个好处：很多尺寸在编译期已知，编译器可以展开循环、分配固定数组、优化寄存器使用。高性能 GEMM 很依赖这种静态信息。
...
> **导读补充**
>
> 如果这些操作单独开一个 kernel，就要把 C 写到 GMEM，再从 GMEM 读回来处理，再写回。Epilogue 的目的就是在 GEMM 写回前顺手完成这些轻量后处理，减少额外内存往返。

> **给非专业读者**
>
> 这像菜出锅前顺手加葱花和调味。如果先把菜端到餐桌，再端回厨房加葱花，就多走一趟。
...
> **导读补充**
>
> 这就是流水线思想。不是让单次搬运更快，而是让搬运时间被计算时间遮住一部分。只要 pipeline 稳定，GPU 就更少停下来等 GMEM。

> **给非专业读者**
>
> 工厂流水线上，一个工人装配当前零件，另一个工人已经把下一批零件放到旁边。大家都不停手，产线就顺。
...
> **导读补充**
>
> 所以 CUTLASS 并不是跳过手写优化。它是在手写优化已经被理解之后，把这些重复出现的结构抽出来。你越懂前两篇，越能读懂 CUTLASS。
...
> **给非专业读者**
>
> 如果每种需求都重新手写一个 kernel，就像每个订单都重新设计一条生产线，成本太高。CUTLASS 的做法是把生产线拆成标准模块：仓库搬运模块、车间缓存模块、工位分配模块、最后包装模块。客户换需求时，不是从零造，而是换一组模块参数。

> **深入一点**
>
> 这也是模板库的价值：不是让一个 kernel 适合所有场景，而是让生成很多专用 kernel 的成本变低。高性能往往来自“专用”，工程效率来自“模板化生成专用”。
...
[打开本站可视化](../cutlass-hierarchy/index.html)
[回到原文](https://developer.nvidia.com/blog/cutlass-linear-algebra-cuda/)
